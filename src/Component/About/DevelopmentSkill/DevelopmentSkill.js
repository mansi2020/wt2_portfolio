...
const DevelopmentSkill = () => {
  ...
  return (
    <>
      <h1 style={{color:"white",textAlign:"center",marginBottom:"2rem"}}>Development Skills</h1>
      <div className="development-skill-container">
        <div className="development-skill-carasouel">
        {skillData.map((item, index) => <img key={index} src={item.url} alt="" />)}
          {skillData.map((item, index) => <img key={index + skillData.length} src={item.url} alt="" />)}
        </div>
      </div>
    </>
  );
